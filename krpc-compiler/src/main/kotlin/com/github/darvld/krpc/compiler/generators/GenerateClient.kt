...
internal fun TypeSpec.Builder.overrideServiceMethod(method: ServiceMethodDefinition) {
    addFunction(method.declaredName, KModifier.OVERRIDE) {
        markAsGenerated()
        
        if (method.isSuspending) addModifiers(KModifier.SUSPEND)
        if (method.requestType != UnitClassName) addParameter(method.requestName, method.requestType)
